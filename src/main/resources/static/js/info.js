...
function sliceDiv(superDiv, data, textFontSize, location) {
	...
	const textWidth = getTextWidth(arrays[0],textFontSize,window.getComputedStyle(infoBody).fontFamily) + 5;
...
function makeMonsterInfo(data) {
	...
	monsterName.textContent = data[0];
	monsterName.style.fontSize = nameFontSize + 'px';
	monsterName.style.position = 'absolute';
	monsterName.style.left = (427 - nameWidth) / 2 + 'px';
	monsterName.style.top = '5px';
	monsterName.style.width = nameWidth + 'px';
	monsterName.style.color = "#ffffff";

	const monsterHP = document.createElement('div');
	monsterHP.textContent = "HP:"+data[1];
	monsterHP.style.fontSize = textFontSize + 'px';
	monsterHP.style.position = 'absolute';
	monsterHP.style.left = '5px';
	monsterHP.style.top = '35px';
	monsterHP.style.color = "#ffffff";
	monsterHP.style.whiteSpace = 'nowrap';
	
	const monsterOverHP = document.createElement('div');
	monsterOverHP.textContent = "オーバーHP:"+data[2];
	monsterOverHP.style.fontSize = textFontSize + 'px';
	monsterOverHP.style.position = 'absolute';
	monsterOverHP.style.left = '100px';
	monsterOverHP.style.top = '35px';
	monsterOverHP.style.color = "#ff7f27";
	monsterOverHP.style.whiteSpace = 'nowrap';

	const monsterAttack = document.createElement('div');
	monsterAttack.textContent = "攻撃力:"+data[3];
	monsterAttack.style.fontSize = textFontSize + 'px';
	monsterAttack.style.position = 'absolute';
	monsterAttack.style.left = '300px';
	monsterAttack.style.top = '35px';
	monsterAttack.style.color = "#ffffff";
	monsterAttack.style.whiteSpace = 'nowrap';
	
	const monsterEXP = document.createElement('div');
	//monsterEXP.id = 'monsterEXP';
	monsterEXP.textContent = "経験値:" + data[4];
	monsterEXP.style.fontSize = textFontSize + 'px';
	monsterEXP.style.position = 'absolute';
	monsterEXP.style.left = '5px';
	monsterEXP.style.top = parseInt(monsterHP.style.top) +
		textFontSize +
		'px';
		monsterEXP.style.color = "#ffffff";
	monsterEXP.style.whiteSpace = 'nowrap';

	const monsterGold = document.createElement('div');
	monsterGold.textContent = "お金:" + data[5];
	monsterGold.style.fontSize = textFontSize + 'px';
	monsterGold.style.position = 'absolute';
	monsterGold.style.left = '130px';
	monsterGold.style.top = parseInt(monsterHP.style.top) +
		textFontSize +
		'px';
		monsterGold.style.color = "#ffffff";
	monsterGold.style.whiteSpace = 'nowrap';

	const monsterTurn = document.createElement('div');
	monsterTurn.textContent = "ボーナスターン:" + data[6];
	monsterTurn.style.fontSize = textFontSize + 'px';
	monsterTurn.style.position = 'absolute';
	monsterTurn.style.left = '240px';
	monsterTurn.style.color = "#ff7f27";
	monsterTurn.style.top = parseInt(monsterHP.style.top) +
		textFontSize +
		'px';
	monsterTurn.style.whiteSpace = 'nowrap';

	infoBody.appendChild(monsterName);
	infoBody.appendChild(monsterHP);
	infoBody.appendChild(monsterOverHP);
	infoBody.appendChild(monsterAttack);
	infoBody.appendChild(monsterEXP);
	infoBody.appendChild(monsterGold);
	infoBody.appendChild(monsterTurn);
	const superDiv = document.createElement('div');
	superDiv.className = 'infoText';
	superDiv.style.left = '5px';
	superDiv.style.maxWidth = "417px";
